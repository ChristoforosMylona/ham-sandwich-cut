...
class HamInstance:
    ...
    def __init__(self, red_points, blue_points, plot_constant=10, steps_taken=None):
        self.red_points = [Point(i) for i in red_points]
        self.blue_points = [Point(i) for i in blue_points]
        ...
        # Calculate Duals
        self.red_duals = [
            compute_dual_line(i, self.plot_constant) for i in self.red_points
        ]
        self.blue_duals = [
            compute_dual_line(i, self.plot_constant) for i in self.blue_points
        ]
            ...
            steps_taken.append(
                {
                    ...
                    "data": {
                        "dualLines": {
                            ...
                            "blue": [line.to_dict() for line in self.blue_duals],
                        }
                    },
                }
            )
    ...
    def start_input(self):
        check = input(
            "You have not provided a point file. Do you want to input points manually [i] or use random points [r]? "
        )
        if check == "i":
            self.red_input()
        elif check == "r":
    ...
    def read_points(self, infile):
        with open(infile, "r") as f:
            ...
            self.red_points = [
                Point(float(i.split(",")[0]), float(i.split(",")[1])) for i in red_line
            ]
            self.blue_points = [
                Point(float(i.split(",")[0]), float(i.split(",")[1])) for i in blue_line
            ]
